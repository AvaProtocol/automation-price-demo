...
function Swap() {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      const queryResponse = await subqueryHelper.getPrices();
      const assetPrices = _.filter(queryResponse, ({ args: { asset } }) => asset === 'mgx:ksm');

      const formattedRows = _.map(assetPrices, (price) => ({ asset: price.args.asset, value: price.args.value, timestamp: formatDatetime(price.timestamp) }));
  ...
  const getTriggerPercentage = () => {
    ...
    const triggerPercentage = Math.ceil(Math.abs(priceVolatility * 100));
                    ...
                    <Col span={24}>
                      <Form.Item label="Stop">
                        <Row>
                          ...
                          <Col span={14}>
                            <span style={{ paddingLeft: 12, lineHeight: '32px' }}> MGX</span>
                            { volatilityElement }
                          </Col>
                        </Row>
                      </Form.Item>
                        ...
                        <Row>
                          <Col span={16}>
                            <span style={{ paddingLeft: 12 }}>
                              {_.isNaN(expectedOutput) ? '-' : expectedOutput}
                              {' '}
                              KSM
                            </span>
                          </Col>
