...
function Tvl() {
  ...
  useEffect( () => {
    setInterval(async () => {
      let allPrices = await subqueryHelper.getPrices();
      ...
      _.each(prices, (price) => price.timestamp = moment.utc(price.timestamp).format());
  ...
  const priceRows = _.map(prices, (priceItem) => {
    ...
    return (
      <tr key={moment(timestamp).toString()} className="price-row">
        ...
        <th className="price-col">${(convertToNumber(value)/10**6).toFixed(1)}MM</th>
      </tr>
    )
  });
